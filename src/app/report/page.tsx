...
export default function ReportPage() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      console.log('Raw response text:', rawText); // Debugging

      // Remove any text before/after the JSON (like ```json, etc.)
      const jsonMatch = rawText.match(/\{[\s\S]*\}/); // Match the JSON object
      if (!jsonMatch) {
        throw new Error('No valid JSON object found in the response.');
      }

      const sanitizedText = jsonMatch[0]; // Extract only the JSON part
      ...
      if (
        ...
        throw new Error('Parsed JSON does not meet the expected structure.');
      }

      // Set the verification result and status
